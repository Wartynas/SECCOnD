...
setwd("C:/Users/marty/Desktop/Imperial/summer_project/hpc_stuff/step1_5percent_unrestricted/3D")

source("../../../utils.R")

# Load main files

NMI_hmap_vanilla_5perc <- readRDS("step_1_5percent_unrestricted_3D_output/intermediate_NMI_hmap_vanilla_5perc.rds")
NMI_hmap_calibrated_5perc <- readRDS("step_1_5percent_unrestricted_3D_output/intermediate_NMI_hmap_calibrated_5perc.rds")
NMI_hmap_consensus_5perc <- readRDS("step_1_5percent_unrestricted_3D_output/intermediate_NMI_hmap_consensus_5perc.rds")

# Can calculate SD myself from the NMI cubes
# NMI_sd_hmap_vanilla_5perc <- readRDS("step_1_5percent_unrestricted_3D_output/intermediate_NMI_sd_hmap_vanilla_5perc.rds")
# NMI_sd_hmap_calibrated_5perc <- readRDS("step_1_5percent_unrestricted_3D_output/intermediate_NMI_sd_hmap_calibrated_5perc.rds")
# NMI_sd_hmap_consensus_5perc <- readRDS("step_1_5percent_unrestricted_3D_output/intermediate_NMI_sd_hmap_consensus_5perc.rds")

nc_hmap_vanilla_5perc <- readRDS("step_1_5percent_unrestricted_3D_output/intermediate_nc_hmap_vanilla_5perc.rds")
nc_hmap_calibrated_5perc <- readRDS("step_1_5percent_unrestricted_3D_output/intermediate_nc_hmap_calibrated_5perc.rds")
nc_hmap_consensus_5perc <- readRDS("step_1_5percent_unrestricted_3D_output/intermediate_nc_hmap_consensus_5perc.rds")

# Load rest of the files (nu_within from 0.97)
...
NMI_hmap_vanilla_5perc[96:nrow(NMI_hmap_vanilla_5perc),,] <- rest_NMI_hmap_vanilla_5perc 
NMI_hmap_calibrated_5perc[96:nrow(NMI_hmap_calibrated_5perc),,] <- rest_NMI_hmap_calibrated_5perc
NMI_hmap_consensus_5perc[96:nrow(NMI_hmap_consensus_5perc),,] <- rest_NMI_hmap_consensus_5perc

nc_hmap_vanilla_5perc[96:nrow(nc_hmap_vanilla_5perc),,] <- rest_nc_hmap_vanilla_5perc
nc_hmap_calibrated_5perc[96:nrow(nc_hmap_calibrated_5perc),,] <- rest_nc_hmap_calibrated_5perc
nc_hmap_consensus_5perc[96:nrow(nc_hmap_consensus_5perc),,] <- rest_nc_hmap_consensus_5perc
...
mean_NMI_2D_vanilla <- apply(NMI_hmap_vanilla_5perc, MARGIN = c(1,2), FUN = mean) %>% round(digits = 3)
mean(mean_NMI_2D_vanilla)
mean_NMI_2D_cal <- apply(NMI_hmap_calibrated_5perc, MARGIN = c(1,2), FUN = mean) %>% round(digits = 3)
mean(mean_NMI_2D_cal)
mean_NMI_2D_con <- apply(NMI_hmap_consensus_5perc, MARGIN = c(1,2), FUN = mean) %>% round(digits = 3)
mean(mean_NMI_2D_con)

sd_NMI_2D_vanilla <- apply(NMI_hmap_vanilla_5perc, MARGIN = c(1,2), FUN = sd) %>% round(digits = 3)
sd_NMI_2D_cal <- apply(NMI_hmap_calibrated_5perc, MARGIN = c(1,2), FUN = sd) %>% round(digits = 3)
sd_NMI_2D_con <- apply(NMI_hmap_consensus_5perc, MARGIN = c(1,2), FUN = sd) %>% round(digits = 3)

mode_nc_2D_vanilla <- apply(nc_hmap_vanilla_5perc, MARGIN = c(1,2), FUN = getmode)
mode_nc_2D_cal <- apply(nc_hmap_calibrated_5perc, MARGIN = c(1,2), FUN = getmode)
mode_nc_2D_con <- apply(nc_hmap_consensus_5perc, MARGIN = c(1,2), FUN = getmode)
...
my_levelplot <- function(matrix_2d, num_of_colors, legend_title, binary = FALSE) {
  if (binary) {
    levelplot(
      ...
      main = list(TeX("$p=5\\%$", italic = TRUE), cex = 3),
      colorkey=list(
        height=0.1,
        at = seq(0, 1, 0.5),
        labels = list(
...
for (i in 1:99) {
  for (j in 1:99) {
    if (mean(NMI_hmap_vanilla_5perc[i,j,]) != mean(NMI_hmap_calibrated_5perc[i,j,])) {
      vanilla_cal_pvalues[i,j] <- t.test(NMI_hmap_vanilla_5perc[i,j,], NMI_hmap_calibrated_5perc[i,j,])$p.value
    }
  }
}
...
for (i in 1:99) {
  for (j in 1:99) {
    if (mean(NMI_hmap_vanilla_5perc[i,j,]) != mean(NMI_hmap_consensus_5perc[i,j,])) {
      vanilla_con_pvalues[i,j] <- t.test(NMI_hmap_vanilla_5perc[i,j,], NMI_hmap_consensus_5perc[i,j,])$p.value
    }
  }
}
