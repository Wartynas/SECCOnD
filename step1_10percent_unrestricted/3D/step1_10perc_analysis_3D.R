...
setwd("C:/Users/marty/Desktop/Imperial/summer_project/hpc_stuff/step1_10percent_unrestricted/3D")

source("../../../utils.R")

# Load main files

NMI_hmap_vanilla_10perc <- readRDS("step_1_10percent_unrestricted_3D_output/intermediate_NMI_hmap_vanilla_10perc.rds")
NMI_hmap_calibrated_10perc <- readRDS("step_1_10percent_unrestricted_3D_output/intermediate_NMI_hmap_calibrated_10perc.rds")
NMI_hmap_consensus_10perc <- readRDS("step_1_10percent_unrestricted_3D_output/intermediate_NMI_hmap_consensus_10perc.rds")

# Can calculate SD myself from the NMI cubes
# NMI_sd_hmap_vanilla_10perc <- readRDS("step_1_10percent_unrestricted_3D_output/intermediate_NMI_sd_hmap_vanilla_10perc.rds")
# NMI_sd_hmap_calibrated_10perc <- readRDS("step_1_10percent_unrestricted_3D_output/intermediate_NMI_sd_hmap_calibrated_10perc.rds")
# NMI_sd_hmap_consensus_10perc <- readRDS("step_1_10percent_unrestricted_3D_output/intermediate_NMI_sd_hmap_consensus_10perc.rds")

nc_hmap_vanilla_10perc <- readRDS("step_1_10percent_unrestricted_3D_output/intermediate_nc_hmap_vanilla_10perc.rds")
nc_hmap_calibrated_10perc <- readRDS("step_1_10percent_unrestricted_3D_output/intermediate_nc_hmap_calibrated_10perc.rds")
nc_hmap_consensus_10perc <- readRDS("step_1_10percent_unrestricted_3D_output/intermediate_nc_hmap_consensus_10perc.rds")

# Load rest of the files (nu_within from 0.52)

rest_NMI_hmap_vanilla_10perc <- readRDS("rest/intermediate_NMI_hmap_vanilla_10perc.rds")
rest_NMI_hmap_calibrated_10perc <- readRDS("rest/intermediate_NMI_hmap_calibrated_10perc.rds")
rest_NMI_hmap_consensus_10perc <- readRDS("rest/intermediate_NMI_hmap_consensus_10perc.rds")
...
rest_nc_hmap_vanilla_10perc <- readRDS("rest/intermediate_nc_hmap_vanilla_10perc.rds")
rest_nc_hmap_calibrated_10perc <- readRDS("rest/intermediate_nc_hmap_calibrated_10perc.rds")
rest_nc_hmap_consensus_10perc <- readRDS("rest/intermediate_nc_hmap_consensus_10perc.rds")

# Merging the files

NMI_hmap_vanilla_10perc[51:nrow(NMI_hmap_vanilla_10perc),,] <- rest_NMI_hmap_vanilla_10perc 
NMI_hmap_calibrated_10perc[51:nrow(NMI_hmap_calibrated_10perc),,] <- rest_NMI_hmap_calibrated_10perc
NMI_hmap_consensus_10perc[51:nrow(NMI_hmap_consensus_10perc),,] <- rest_NMI_hmap_consensus_10perc

nc_hmap_vanilla_10perc[51:nrow(nc_hmap_vanilla_10perc),,] <- rest_nc_hmap_vanilla_10perc
nc_hmap_calibrated_10perc[51:nrow(nc_hmap_calibrated_10perc),,] <- rest_nc_hmap_calibrated_10perc
nc_hmap_consensus_10perc[51:nrow(nc_hmap_consensus_10perc),,] <- rest_nc_hmap_consensus_10perc
...
mean_NMI_2D_vanilla <- apply(NMI_hmap_vanilla_10perc, MARGIN = c(1,2), FUN = mean) %>% round(digits = 3)
mean(mean_NMI_2D_vanilla)
mean_NMI_2D_cal <- apply(NMI_hmap_calibrated_10perc, MARGIN = c(1,2), FUN = mean) %>% round(digits = 3)
mean(mean_NMI_2D_cal)
mean_NMI_2D_con <- apply(NMI_hmap_consensus_10perc, MARGIN = c(1,2), FUN = mean) %>% round(digits = 3)
mean(mean_NMI_2D_con)

sd_NMI_2D_vanilla <- apply(NMI_hmap_vanilla_10perc, MARGIN = c(1,2), FUN = sd) %>% round(digits = 3)
sd_NMI_2D_cal <- apply(NMI_hmap_calibrated_10perc, MARGIN = c(1,2), FUN = sd) %>% round(digits = 3)
sd_NMI_2D_con <- apply(NMI_hmap_consensus_10perc, MARGIN = c(1,2), FUN = sd) %>% round(digits = 3)

mode_nc_2D_vanilla <- apply(nc_hmap_vanilla_10perc, MARGIN = c(1,2), FUN = getmode)
mode_nc_2D_cal <- apply(nc_hmap_calibrated_10perc, MARGIN = c(1,2), FUN = getmode)
mode_nc_2D_con <- apply(nc_hmap_consensus_10perc, MARGIN = c(1,2), FUN = getmode)
...
my_levelplot <- function(matrix_2d, num_of_colors, legend_title, binary = FALSE) {
  if (binary) {
    levelplot(
      ...
      main = list(TeX("$p=10\\%$", italic = TRUE), cex = 3),
      colorkey=list(
        height=0.1,
        at = seq(0, 1, 0.5),
        labels = list(
...
for (i in 1:99) {
  for (j in 1:99) {
    if (mean(NMI_hmap_vanilla_10perc[i,j,]) != mean(NMI_hmap_calibrated_10perc[i,j,])) {
      vanilla_cal_pvalues[i,j] <- t.test(NMI_hmap_vanilla_10perc[i,j,], NMI_hmap_calibrated_10perc[i,j,])$p.value
    }
  }
}
...
for (i in 1:99) {
  for (j in 1:99) {
    if (mean(NMI_hmap_vanilla_10perc[i,j,]) != mean(NMI_hmap_consensus_10perc[i,j,])) {
      vanilla_con_pvalues[i,j] <- t.test(NMI_hmap_vanilla_10perc[i,j,], NMI_hmap_consensus_10perc[i,j,])$p.value
    }
  }
}
...
dif_con_van <- NMI_hmap_consensus_10perc - NMI_hmap_vanilla_10perc
#dif_no_nas[is.na(dif_no_nas)] <- 0
heatmap.2(dif_con_van, dendrogram = "none",  Rowv = NA, Colv = NA, na.color = "grey", trace= "none")

dif_cal_van <- NMI_hmap_calibrated_10perc - NMI_hmap_vanilla_10perc
