...
PERCENT_OF_EDGES_TO_REWIRE <- 10 # - try 1, 5, 10 and 20.
PK = c(25,25,25,25) # composition of the network
MAX_NC <- sum(PK)/2
NU_WITHIN <- seq(0.01,1,0.01)
NU_BETWEEN <- seq(0.01,1,0.01)
...
NMI_hmap_vanilla_10perc <- matrix(NA, nrow = length(NU_WITHIN), ncol = length(NU_BETWEEN))
NMI_hmap_calibrated_10perc <- matrix(NA, nrow = length(NU_WITHIN), ncol = length(NU_BETWEEN))
NMI_hmap_consensus_10perc <- matrix(NA, nrow = length(NU_WITHIN), ncol = length(NU_BETWEEN))

NMI_sd_hmap_vanilla_10perc <- matrix(NA, nrow = length(NU_WITHIN), ncol = length(NU_BETWEEN))
NMI_sd_hmap_calibrated_10perc <- matrix(NA, nrow = length(NU_WITHIN), ncol = length(NU_BETWEEN))
NMI_sd_hmap_consensus_10perc <- matrix(NA, nrow = length(NU_WITHIN), ncol = length(NU_BETWEEN))

nc_hmap_vanilla_10perc <- matrix(NA, nrow = length(NU_WITHIN), ncol = length(NU_BETWEEN))
nc_hmap_calibrated_10perc <- matrix(NA, nrow = length(NU_WITHIN), ncol = length(NU_BETWEEN))
nc_hmap_consensus_10perc <- matrix(NA, nrow = length(NU_WITHIN), ncol = length(NU_BETWEEN))
  ...
  for(m in 1:length(NU_BETWEEN)) {
    ...
    if (check_if_community_structure_is_likely(PK, nu_with, nu_betw) < 1) {
      ...
      gathered_output <- foreach (
        ...
        c(vanilla_nmi, vanilla_nc, cal_nmi, cal_nc, con_nmi, con_nc) # this is what 1 core would return
      }
      end <- Sys.time()
      print(paste0("Combination of nu_within = ", nu_with, " and nu_between = ", nu_betw, " took ", end-start))
      
      NMI_hmap_vanilla_10perc[n,m] <- mean(gathered_output[,1])
      NMI_hmap_calibrated_10perc[n,m] <- mean(gathered_output[,3])
      NMI_hmap_consensus_10perc[n,m] <- mean(gathered_output[,5])
      
      NMI_sd_hmap_vanilla_10perc[n,m] <- sd(gathered_output[,1])
      NMI_sd_hmap_calibrated_10perc[n,m] <- sd(gathered_output[,3])
      NMI_sd_hmap_consensus_10perc[n,m] <- sd(gathered_output[,5])
      
      nc_hmap_vanilla_10perc[n,m] <- getmode(gathered_output[,2])
      nc_hmap_calibrated_10perc[n,m] <- getmode(gathered_output[,4])
      nc_hmap_consensus_10perc[n,m] <- getmode(gathered_output[,6])
      
      saveRDS(NMI_hmap_vanilla_10perc, "/rds/general/user/mv321/home/thesis/intermediate_NMI_hmap_vanilla_10perc.rds")
      saveRDS(NMI_hmap_calibrated_10perc, "/rds/general/user/mv321/home/thesis/intermediate_NMI_hmap_calibrated_10perc.rds")
      saveRDS(NMI_hmap_consensus_10perc, "/rds/general/user/mv321/home/thesis/intermediate_NMI_hmap_consensus_10perc.rds")
      
      saveRDS(NMI_sd_hmap_vanilla_10perc, "/rds/general/user/mv321/home/thesis/intermediate_NMI_sd_hmap_vanilla_10perc.rds")
      saveRDS(NMI_sd_hmap_calibrated_10perc, "/rds/general/user/mv321/home/thesis/intermediate_NMI_sd_hmap_calibrated_10perc.rds")
      saveRDS(NMI_sd_hmap_consensus_10perc, "/rds/general/user/mv321/home/thesis/intermediate_NMI_sd_hmap_consensus_10perc.rds")
      
      saveRDS(nc_hmap_vanilla_10perc, "/rds/general/user/mv321/home/thesis/intermediate_nc_hmap_vanilla_10perc.rds")
      saveRDS(nc_hmap_calibrated_10perc, "/rds/general/user/mv321/home/thesis/intermediate_nc_hmap_calibrated_10perc.rds")
      saveRDS(nc_hmap_consensus_10perc, "/rds/general/user/mv321/home/thesis/intermediate_nc_hmap_consensus_10perc.rds")
      
    }
  }
}

saveRDS(NMI_hmap_vanilla_10perc, "/rds/general/user/mv321/home/thesis/NMI_hmap_vanilla_10perc.rds")
saveRDS(NMI_hmap_calibrated_10perc, "/rds/general/user/mv321/home/thesis/NMI_hmap_calibrated_10perc.rds")
saveRDS(NMI_hmap_consensus_10perc, "/rds/general/user/mv321/home/thesis/NMI_hmap_consensus_10perc.rds")

saveRDS(NMI_sd_hmap_vanilla_10perc, "/rds/general/user/mv321/home/thesis/NMI_sd_hmap_vanilla_10perc.rds")
saveRDS(NMI_sd_hmap_calibrated_10perc, "/rds/general/user/mv321/home/thesis/NMI_sd_hmap_calibrated_10perc.rds")
saveRDS(NMI_sd_hmap_consensus_10perc, "/rds/general/user/mv321/home/thesis/NMI_sd_hmap_consensus_10perc.rds")

saveRDS(nc_hmap_vanilla_10perc, "/rds/general/user/mv321/home/thesis/nc_hmap_vanilla_10perc.rds")
saveRDS(nc_hmap_calibrated_10perc, "/rds/general/user/mv321/home/thesis/nc_hmap_calibrated_10perc.rds")
saveRDS(nc_hmap_consensus_10perc, "/rds/general/user/mv321/home/thesis/nc_hmap_consensus_10perc.rds")
